...
The shipped `typescript` pack uses the same manifest-driven path: tsconfig and first-party TypeScript signals route to `bill-typescript-code-review` and `bill-typescript-code-check`, with governed native agents for the baseline and all ten specialist lanes. Its quality checks understand package-manager scripts, `tsc`, ESLint, Biome, Prettier, Vitest/Jest, workspaces, turbo, and nx; TypeScript-specific routing is not hard-coded into either generic shell.
