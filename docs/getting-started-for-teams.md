...
Skill Bill gives a team governed agent workflows, not just prompt files. Expect two beneficiary groups, and plan the rollout for both: developers for whom raw agents never clicked — the governed process is what turns them into agent users at all — and experienced agent users, who tend to stay for the parts a prompt folder cannot give them: agent-independent resume, per-repo memory in boundary `history.md` and decision logs, and packs tuned through overrides instead of forks.

There are two useful adoption modes:
...
[Skill Bill Teams Roadmap](team-control-plane-roadmap.md). That roadmap keeps
positioning and product principles only, and lists the questions the local
governance model has to answer before any SaaS control plane is built.
...
|---------|-------------|----------------|
| `/bill-feature-spec` | Preparing specs before implementation starts | Parent spec, one-or-more executable subtasks, and an authoritative manifest |
...
| `/bill-code-review` | Reviewing staged changes, a PR, or a commit range | Routed review with summary, risk register, action items, and verdict; accepts `mode:auto|inline|delegated` |
| `/bill-code-check` | Running repo checks before a PR | Dominant pack `validation_gate` collect-all argv in-session |
...
For a standalone review, use `/bill-code-review mode:inline` to run the reduced-depth review in one `bill-code-review-inline` session (broker-paged evidence, no specialist fan-out, not equivalent coverage to delegated) — or `/bill-code-review mode:delegated` to require the specialist fan-out. Omitting the argument and `mode:auto` both resolve to inline. Feature and goal workflows review inline (`/bill-feature <issue-key> code-review:auto|inline`); delegated review is a standalone `/bill-code-review` launch.
...
On Claude, Codex, Cursor, and Junie, shipped platform-pack baseline and specialist reviewers can install native subagent definitions where the source pack declares them. These stack-specific review skills are **internal sidecars** of `/bill-code-review` — they are not listed slash commands; invoke code review through `/bill-code-review`, which routes to the matching stack. Invoke quality checks through `/bill-code-check`, which runs the dominant pack `validation_gate` argv in-session. Each declared review bundle registers the baseline and specialist reviewers as native subagents, so spawn-based execution can cover both layers. Native subagent sources live as provider-neutral `native-agents/agents.yaml` bundles or standalone `native-agents/<name>.md` files. New and rendered neutral sources include `contract_version: "0.1"`; parser tolerance for older unpinned sources is migration support, not a reason to omit the pin in new source. Install renders the same sources into `~/.skill-bill/native-agents/` before linking Claude markdown, Codex TOMLs, and Junie markdown into each runtime's native agents directory. Codex TOMLs normally install to `~/.codex/agents/`; `~/.agents/agents/` is only a Skill Bill compatibility path for homes without `.codex`. Generated provider files are install-cache outputs, not committed source. The orchestrator's spawn prose ("spawn the `bill-kmp-code-review-ui` subagent", "spawn the `bill-kotlin-code-review-architecture` subagent", "spawn the `bill-python-code-review-security` subagent", and so on) is runtime-neutral: Claude and Junie resolve the installed Markdown/YAML custom subagents, and Codex resolves each TOML by `name`. Feature-task and goal execution use the Kotlin runtime driver; `bill-feature-verify` has no verify-specific native subagents and delegates review through `bill-code-review` while keeping feature-flag, completeness, and verdict audits inline. Feature-task and goal workflows resume from durable runtime-owned workflow state, and that resume is agent-independent: a run paused under one agent continues under another by launching `bill-feature <KEY>` there — verified in extended real-world use across Claude Code and Codex; other agents follow their support tiers.

For manifest-backed feature goals, including one-subtask goals, teach operators to trust the runtime-owned flat
...
`skill-bill goal watch <issue-key> --interval-seconds 5` prints every refresh,
including unchanged status, and follows until the goal
...
Go, iOS, Kotlin, PHP, Python, Rust, and TypeScript directly declare all ten approved review areas. KMP covers Android and Kotlin Multiplatform with seven declared areas — `architecture`, `platform-correctness`, `security`, `persistence`, `reliability`, `ui`, and `ux-accessibility` — composing the remaining three (`performance`, `testing`, `api-contracts`) from the Kotlin baseline. Review composition never supplies quality behavior: each dominant pack declares a `validation_gate`, and `bill-code-check` runs the winning pack's argv without a checker sidecar or fallback.
...
Externally authored platform packs are the kind of evidence this framework is designed to support. A pack does not need to start as a bundled reference pack for the authoring path to be successful.
