...
6. Emit JSON payload. When the user accepts, build a payload shaped like:
   ...
   For orchestrator kinds (`horizontal`, `platform-override-piloted`, `platform-pack`), the scaffolder emits one provider-neutral source bundle for names listed in `subagent_specialists`. Source stubs land in `<orchestrator-skill-dir>/native-agents/agents.yaml`; generated Claude, Codex, OpenCode, and Junie artifacts are rendered into the install cache when users install the skill. The scaffolder also injects a `## Subagent Spawn Runtime Notes` section into the orchestrator's `content.md`. Set `no_subagents: true` to opt out explicitly when an orchestrator should not delegate to native subagents (the two flags cannot both be set together). Specialist names must match `^[a-z][a-z0-9-]*$`, must be unique, and are NOT valid for leaf kinds such as `code-review-area` or `add-on`. After scaffolding, fill in TODO placeholders in `native-agents/agents.yaml`, then run `skill-bill render` to validate sources before shipping.
