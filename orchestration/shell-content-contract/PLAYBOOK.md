...
This is the canonical shell+content contract. Governed platform-pack source
uses `content.md` for authored execution content. Runtime `SKILL.md` wrappers
and platform pointer files are generated by render/install flows; they are not
source-authored files. Generated support pointers such as `shell-ceremony.md`
carry project-overrides, execution-mode reporting, and telemetry rules in
rendered or installed output.

Installed skills consume this file through generated sibling support pointers
(e.g. `shell-content-contract.md` inside the staged skill directory), so agents
read canonical rules without source skill directories committing generated
files.

Do not reference this repo-relative path directly from installable skills — use
the generated sibling support pointer instead.
...
Every path declared in `declared_files` must resolve to a source `content.md`
file on disk relative to the platform pack root.

Each declared content file must be a Markdown file with a YAML frontmatter
block (`---` ... `---`) and clean authored guidance. It must not contain
generated wrapper headings:

- `## Descriptor`
- `## Execution`
- `## Ceremony`

Those sections are generated into the runtime wrapper by `skill-bill render`
and install staging. Rendered wrappers must include all three H2 sections in a
deterministic shape around the authored `content.md` body.

Each governed source skill directory must contain:

- `content.md` — platform-authored execution content rendered into the
  generated wrapper's `## Execution` body.

Source skill directories under `skills/` may also contain `native-agents/` when
they own provider-neutral native-agent sources. They must not commit generated
`SKILL.md` wrappers or support pointer files. Install staging materializes those
runtime files under the configured Skill Bill installed-skills cache.
...
reporting, generated wrapper section headings, or required support-pointer
...
- A file path declared under `declared_files` does not exist →
  ...
  resolved path.
- A declared content file is missing required frontmatter, has invalid
  frontmatter, or contains generated wrapper boilerplate → the corresponding
  shell-content validation error. The message must include the file path.
...
4. Declared `content.md` file presence.
5. Declared `content.md` frontmatter and authored-content validation.
6. Sibling `content.md` presence.
7. Sibling `shell-ceremony.md` presence.
8. Rendered `## Execution` body validation.
