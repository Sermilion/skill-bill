...
class ConfigResolveExternalPlatformPacksCommand(
  ...
  override fun run() {
    ...
    val lines = catalog.entries.map { entry ->
      val shadowed = entry.shadowedBundledSlug ?: "-"
      "${entry.loaded.manifest.slug}\t${entry.loaded.sourceKind.wireValue}\t$shadowed\t${entry.loaded.canonicalRoot}\n"
    }
    state.completeText(
      lines.joinToString(""),
      mapOf(
        SharedPayloadKeys.STATUS to "ok",
        ExternalPlatformPackResolutionPayloadKeys.PACKS to catalog.entries.map { entry ->
          mapOf(
