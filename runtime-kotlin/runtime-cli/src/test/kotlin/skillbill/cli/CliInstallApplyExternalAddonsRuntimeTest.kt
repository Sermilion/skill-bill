...
class CliInstallApplyExternalAddonsRuntimeTest {
  ...
  fun `valid external source applies overlay and reports applied with merged manifest`() {
    ...
    val sourceDir = seedCapmoSource(home)
    ...
    assertTrue(
      Files.isRegularFile(packRoot.resolve("addons/capmo-review.md")),
    ...
    assertContains(mergedManifest, "slug: capmo")
    assertContains(mergedManifest, "target: platform-packs/ios/addons/capmo-review.md")
  }
  ...
  private fun seedCapmoSource(home: Path): Path {
    val sourceDir = Files.createDirectories(home.resolve("private/ios-capmo"))
    Files.writeString(sourceDir.resolve("capmo-review.md"), "# capmo review body\n")
    Files.writeString(
      sourceDir.resolve("addon-manifest.yaml"),
      """
      addon_usage:
