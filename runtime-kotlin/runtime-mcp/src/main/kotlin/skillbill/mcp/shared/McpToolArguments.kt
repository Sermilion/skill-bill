...
internal fun Map<String, Any?>.boolean(name: String): Boolean =
  when (val value = this[name]) {
    null -> false
    is Boolean -> value
...
internal fun Map<String, Any?>.int(
  name: String,
  default: Int,
): Int =
  when (val value = this[name]) {
...
internal fun Map<String, Any?>.optionalInt(name: String): Int? =
  when (val value = this[name]) {
    null -> null
    is Int -> value
    is Long -> {
...
internal fun Map<String, Any?>.optionalMap(name: String): Map<String, Any?>? =
  when {
    !containsKey(name) -> null
      ...
      JsonCodec.anyToStringAnyMap(this[name])
