...
class McpQualityCheckTelemetryNormalizationTest {
  ...
  fun `quality check completion refuses runtime-owned telemetry keys`() {
    ...
    val error =
      context.callToolError("quality_check_finished", mapOf("completion" to "operator_completed"))

    assertEquals("quality_check_finished", error["tool"])
    assertEquals(
      "MCP tool 'quality_check_finished' argument 'completion': is runtime-owned",
      error["error"],
    )
  }
