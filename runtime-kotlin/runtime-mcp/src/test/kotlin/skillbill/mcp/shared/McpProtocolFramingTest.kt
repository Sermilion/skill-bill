...
class McpProtocolFramingTest {
  ...
  fun `stdio and governed evidence modes share parse error and ping framing`() {
    val tempDir = Files.createTempDirectory("skillbill-mcp-framing")
    val component =
      McpRuntimeContext(environment = disabledTelemetryEnvironment(tempDir), userHome = tempDir).mcpComponent()
    val stdioParseError = decode(McpStdioServer.handleLine("{", component))
    val stdioPing =
      decode(McpStdioServer.handleLine("""{"jsonrpc":"2.0","id":1,"method":"ping"}""", component))
    val evidenceParseError =
      decode(
