...
- `INSERT OR IGNORE` plus read-back equality stays exactly as it was; the key gained a `generation` dimension. The read-back must use an exact-generation query, never the generation-fallback read — a fallback there compares a fresh generation's bytes against the prior generation's row and raises a spurious `Conflict`. reusable
- Generation identity is a durable monotonic per-workflow ordinal stored as a workflow artifact. `artifacts` in `workflow-state-schema.yaml` is deliberately open (`additionalProperties: true`), so a new artifact key needs no workflow-state contract bump and quarantines no legacy record. reusable
- No pre-existing durable identity survives the rewind: `currentReviewPassNumber`/`completedReviewPassNumber` are nulled by the reset, and the phase-ledger `sequenceNumber` advances on every crash-replay, so using it would break byte-identical-retry idempotency. Checked before minting a new concept, as SKILL-150 subtask 4 will later widen this seam.
- The ordinal is minted inside the same transaction that writes the tombstone, which has two callers (`RunLoop.drive` and `Runner.reopenCappedReviewOnChangedDelta`), so incrementing at the write seam covers both without per-caller logic. A derive-only "tombstone present means generation 1" signal is wrong: it reverts to 0 once the fresh review overwrites the tombstone and re-arms the collision at a higher attempt. reusable
- Workflows tombstoned before the ordinal existed never re-enter the mint seam, so `reconcileReviewGeneration` backfills them to 1 in-band on their next run. Without it the fix does nothing for exactly the workflows already broken.
- The non-zero generation is scoped to the two phases the reset rewinds, declared as `GENERATION_SCOPED_PHASE_IDS` rather than branched per call. Every other phase keeps a generation-blind key, so a byte-identical planning re-write after a restart stays an idempotent no-op. reusable
- SQLite cannot widen a PRIMARY KEY in place: migration 16 renames, recreates, copies with an explicit column list and a literal 0, then drops. `ensureDatabase` runs `createBaseSchema` before `DatabaseMigrations.apply`, so the migration executes on fresh databases too and its `CREATE TABLE` text is what `sqlite_master` stores — which makes a `sqlite_master.sql` equality assertion between a fresh and a migrated database a real drift guard for the two DDL sources. reusable
- Gotcha: the run-loop test fake stored producer evidence with a bare `putIfAbsent` and no read-back equality, so the suite had been green against a store that could not conflict. A regression fixture for a write-once collision proves nothing until the double mirrors the production guard. reusable
- New `producer-output-evidence-schema.yaml` at 0.1 rather than bumping the diagnostics schema: the diagnostic metadata shape is unchanged and a bump would loud-fail legacy diagnostic records for no reason. The schema omits `payload` entirely so raw bytes never enter a validated instance or a validation message.
- Known limitation: `rejected_output_diagnostics` keeps its generation-blind `rod_sha256(workflow, phase, attempt)` identity. It is stored durably on quarantine entries as `diagnosticIdentity`, so widening it would rewrite already-persisted identities; a restarted generation can still collide there.
- Review findings recorded and not fixed: the generation-fallback read can attribute a prior generation's bytes to the current generation if a generation-scoped read seam ever appears (unreachable today, every read seam resolves to generation 0); `reconcileReviewGeneration` opens a per-run write transaction for a once-per-workflow backfill.
Feature flag: N/A
