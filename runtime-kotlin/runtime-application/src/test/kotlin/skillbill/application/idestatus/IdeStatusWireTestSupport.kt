...
fun IdeStatusSnapshot.toStatusWireMap(): Map<String, Any?> = buildMap {
...
private fun IdeStatusCurrentSubtask.toWireMap(): Map<String, Any?> = buildMap {
  put("id", id)
  startedAt?.let { put("started_at", it.toString()) }
...
private fun IdeStatusCurrentModel.toWireMap(): Map<String, Any?> = buildMap {
  put("model", model)
  effort?.let { put("effort", it) }
  phaseId?.let { put(SharedPayloadKeys.PHASE_ID, it) }
}

private fun IdeStatusPlanning.toWireMap(): Map<String, Any?> = buildMap {
...
private fun IdeStatusCurrentPhaseExecution.toWireMap(): Map<String, Any?> = buildMap {
...
private fun Map<String, Any?>.plusIfNotNull(key: String, value: Any?): Map<String, Any?> =
