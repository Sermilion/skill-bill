...
class LearningService(private val database: DatabaseSessionFactory) {
  ...
  fun add(request: AddLearningInput): LearningRecordResult =
    database.transaction { unitOfWork ->
      val createRequest =
  ...
  fun edit(request: EditLearningInput): LearningRecordResult =
    database.transaction { unitOfWork ->
      val record =
  ...
  fun delete(id: Int): LearningDeleteResult =
    database.transaction { unitOfWork ->
