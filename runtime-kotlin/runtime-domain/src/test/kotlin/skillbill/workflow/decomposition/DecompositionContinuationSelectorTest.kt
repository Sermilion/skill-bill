...
class DecompositionContinuationSelectorTest {
  ...
  fun `select resumes in-progress subtask at last resumable step`() {
    val selection =
      DecompositionContinuationSelector.select(
        manifest(
  ...
  private fun blockedSubtask(
    id: Int,
    reason: String,
  ): DecompositionSubtask = subtask(id, status = "blocked").copy(blockedReason = reason)
}
