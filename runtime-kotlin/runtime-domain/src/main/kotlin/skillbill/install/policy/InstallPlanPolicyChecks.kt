...
internal fun requireNoDuplicateAgentTargets(
  label: String,
  targets: List<InstallAgentTarget>,
) {
  val duplicates =
    targets
      .groupBy { target -> target.agent to target.path.normalized() }
...
internal fun requireNoDuplicateSkills(
  label: String,
...
private fun duplicateSkills(skills: List<InstallPlanSkill>): Map<String, List<InstallPlanSkill>> =
  skills
    .groupBy(InstallPlanSkill::name)
    .filterValues { matchingSkills -> matchingSkills.size > 1 }

private fun duplicateSkillMessage(
  prefix: String,
  duplicates: Map<String, List<InstallPlanSkill>>,
): String =
