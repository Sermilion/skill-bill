...
internal fun Map<String, Any?>.requireSubtasks(): List<FeatureTaskRuntimeDecomposeSubtask> {
  val rawSubtasks =
    this[DecompositionPlanningPayloadKeys.SUBTASKS] as? List<*>
  ...
  return rawSubtasks.mapIndexed { index, raw ->
    ...
    FeatureTaskRuntimeDecomposeSubtask(
      id = subtask.requireInt(DecompositionPlanningPayloadKeys.ID, index),
      name = subtask.requireString(DecompositionPlanningPayloadKeys.NAME, index),
      scope = subtask.requireString(DecompositionPlanningPayloadKeys.SCOPE, index),
      ...
      dependsOn = subtask.optionalIntList(DecompositionPlanningPayloadKeys.DEPENDS_ON, index),
      linearIssueId =
        subtask[DecompositionPlanningPayloadKeys.LINEAR_ISSUE_ID]?.let { raw ->
...
internal fun Map<String, Any?>.stringAnyMap(key: String): Map<String, Any?>? =
