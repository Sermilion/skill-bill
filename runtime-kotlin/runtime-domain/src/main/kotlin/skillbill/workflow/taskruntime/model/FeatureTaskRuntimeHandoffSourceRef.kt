...
sealed interface FeatureTaskRuntimeHandoffSourceRef {
  ...
  fun toDeclarationMap(): Map<String, String> = when (this) {
    is UpstreamPhaseOutput -> mapOf("kind" to "upstream_phase_output", DecompositionPlanningPayloadKeys.ID to producingPhaseId)
    is RunInvariantField -> mapOf("kind" to "run_invariant_field", DecompositionPlanningPayloadKeys.ID to invariantField.wireValue)
    DerivedCeremonyScaling -> mapOf("kind" to "derived_ceremony_scaling", DecompositionPlanningPayloadKeys.ID to "ceremony_scaling")
    SharedReviewEvidence -> mapOf("kind" to SHARED_REVIEW_EVIDENCE_WIRE, DecompositionPlanningPayloadKeys.ID to SHARED_REVIEW_EVIDENCE_WIRE)
