...
object ReviewFindingFieldCodec {
  fun findingRefOf(
    ...
    sequenceOf(id, findingId, fNumber)
  ...
  fun decodeCitations(raw: Any?): ReviewFindingCitationsDecode {
    ...
    items.forEachIndexed { index, item ->
      ...
      val path =
        (map["path"] as? String)?.trim()?.takeIf(String::isNotBlank)
      ...
      when (val line = parseCitationLine(map["line"])) {
        ...
        is ParsedCitationLine.Rejected ->
          diagnostics +=
  ...
  fun severityAdjustmentOf(raw: Any?): ReviewSeverityAdjustment? {
    ...
    val direction =
      (map["direction"] as? String)?.trim()?.takeIf(String::isNotBlank)
  ...
  private fun parseCitationLine(raw: Any?): ParsedCitationLine =
    when (raw) {
