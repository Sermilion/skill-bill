...
private fun parsePackagedYaml(document: String): Any? =
  try {
...
internal fun Any?.packagedPositiveInt(
  fieldLabel: String,
...
internal fun Any?.packagedPositiveLong(
  fieldLabel: String,
  reject: (String) -> Nothing,
): Long {
...
internal fun requireUniqueStringItems(
  items: List<String>,
  fieldLabel: String,
  reject: (String) -> Nothing,
) {
...
private fun Any?.positiveIntOrNull(): Int? =
  when (this) {
    is Int -> takeIf { it >= 1 }
...
private fun Any?.positiveLongOrNull(): Long? =
  when (this) {
