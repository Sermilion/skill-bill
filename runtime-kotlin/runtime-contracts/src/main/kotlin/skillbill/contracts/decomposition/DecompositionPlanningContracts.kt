...
data class DecompositionPlanningResult(
  ...
  companion object {
    fun fromWireMap(
      ...
      val subtasks = rawSubtasks.mapIndexed { index, raw ->
...
private fun DecompositionPlanningSubtaskWire.toPayload(): Map<String, Any?> = linkedMapOf(
  ...
  DecompositionPlanningPayloadKeys.DEPENDENCIES to dependencies.map { dependency ->
    linkedMapOf(
...
private fun DecompositionPlanningStackBranchWire.toPayload(): Map<String, Any?> = linkedMapOf(
...
private fun Map<String, Any?>.toPlanningSubtask(sourceLabel: String, index: Int): DecompositionPlanningSubtaskWire {
  val name = when {
  ...
  val dependenciesRaw = this[DecompositionPlanningPayloadKeys.DEPENDENCIES]
...
private fun parsePlanningDependencies(
  ...
  return dependencies.mapIndexed { depIndex, value ->
    when (value) {
      ...
      else -> DecompositionPlanningDependencyWire(
...
private fun Map<String, Any?>.stringValue(key: String, sourceLabel: String): String =
...
private fun Map<String, Any?>.nullableStringValue(key: String, sourceLabel: String, subtaskIndex: Int): String? =
...
private fun Map<String, Any?>.optionalIntValue(key: String, sourceLabel: String): Int? =
  if (containsKey(key)) intValue(key, sourceLabel, key) else null

private fun Map<String, Any?>.intValue(key: String, sourceLabel: String, fieldPath: String): Int =
  this[key].asInt(sourceLabel, fieldPath)

private fun Map<String, Any?>.booleanValueOrDefault(key: String, default: Boolean, sourceLabel: String): Boolean =
  when (val value = this[key]) {
    null -> default
    is Boolean -> value
...
private fun invalidPlanning(sourceLabel: String, reason: String): Nothing =
