...
class ExternalAddonOverlayTest {
  ...
  fun `valid merge copies md files and appends entries to installed platform yaml`() {
    ...
    val source = seedExternalSource("ios", "capmo", listOf("capmo-review.md"))
    ...
    assertTrue(Files.isRegularFile(platformPacksRoot.resolve("ios/addons/capmo-review.md")))
    val manifest = Files.readString(platformPacksRoot.resolve("ios/platform.yaml"))
    assertTrue(manifest.contains("slug: capmo"))
    assertTrue(manifest.contains("target: platform-packs/ios/addons/capmo-review.md"))
  }
  ...
  fun `pointer target rewriting converts source-relative target to canonical form`() {
    ...
    val source = seedExternalSource(
      "ios",
      "capmo",
      listOf("capmo-review.md"),
      target = "capmo-review.md",
    )
    ...
    val manifest = Files.readString(platformPacksRoot.resolve("ios/platform.yaml"))
    assertTrue(manifest.contains("target: platform-packs/ios/addons/capmo-review.md"))
    assertFalse(manifest.contains("target: capmo-review.md"))
  }
  ...
  fun `external pointer name colliding with pack owned pointer loud-fails`() {
    seedIosPack(packOwnedAddon = "offline")
    val sourceDir = Files.createDirectories(work.resolve("ext/ios-capmo"))
    Files.writeString(sourceDir.resolve("capmo-review.md"), "# capmo body\n")
    Files.writeString(
    ...
    val manifest = Files.readString(platformPacksRoot.resolve("ios/platform.yaml"))
    assertFalse(manifest.contains("capmo"), "Atomicity: failing overlay must not mutate the installed manifest.")
  }
  ...
  fun `external pointer target colliding with pack owned target basename loud-fails`() {
    seedIosPack(packOwnedAddon = "offline")
    val sourceDir = Files.createDirectories(work.resolve("ext/ios-capmo"))
    Files.writeString(sourceDir.resolve("offline-review.md"), "# capmo overwrite body\n")
    Files.writeString(
      sourceDir.resolve("addon-manifest.yaml"),
      """
      addon_usage:
  ...
  fun `external-vs-external collision across two sources loud-fails`() {
    seedIosPack(packOwnedAddon = null)
    val first = seedExternalSource("ios", "capmo", listOf("capmo-review.md"))
    val secondDir = Files.createDirectories(work.resolve("ext/ios-capmo-2"))
    Files.writeString(secondDir.resolve("capmo-alt.md"), "# alt body\n")
    Files.writeString(
  ...
  fun `missing source md loud-fails`() {
    ...
    Files.writeString(
      ...
      addon_usage:
        code-review/bill-ios-code-review:
          - slug: capmo
            entrypoint: capmo-review.md
      pointers:
        code-review/bill-ios-code-review:
          - name: capmo-review.md
            target: capmo-review.md
      """.trimIndent() + "\n",
    )
  ...
  fun `non-installed platform is skipped with a warning`() {
    seedIosPack(packOwnedAddon = null)
    val source = seedExternalSource("android", "capmo", listOf("capmo-review.md"))
  ...
  fun `multi-source ordering preserves declared config order`() {
    seedIosPack(packOwnedAddon = null)
    seedKotlinPack()
    val iosSource = seedExternalSource("ios", "capmo", listOf("capmo-review.md"))
  ...
  fun `clean analog wipes addons then overlay reconstitutes them`() {
    ...
    assertFalse(Files.exists(addonsDir.resolve("capmo-review.md")))

    overlay.applyOverlay(request(listOf(source)))

    assertTrue(Files.isRegularFile(addonsDir.resolve("capmo-review.md")))
  }

  @Test
  fun `atomicity failing second source leaves first source not applied`() {
    seedIosPack(packOwnedAddon = null)
    val good = seedExternalSource("ios", "capmo", listOf("capmo-review.md"))
    ...
    assertFalse(Files.exists(platformPacksRoot.resolve("ios/addons/capmo-review.md")))
    val manifest = Files.readString(platformPacksRoot.resolve("ios/platform.yaml"))
    assertFalse(manifest.contains("capmo"))
  }
