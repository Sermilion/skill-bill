...
class ClasspathContractSchemaLoaderTest {
  ...
  fun `missing goal progress event schema names classpath resource and does not read cwd`() {
    ...
    val classLoader = object : ClassLoader(ClasspathContractSchemaLoaderTest::class.java.classLoader) {
    ...
    try {
      val error = assertFailsWith<InvalidGoalProgressEventSchemaError> {
