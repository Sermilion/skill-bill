...
class WorkflowRecordMappingTest {
  ...
  fun `phase ledger and progress codecs retain stored timestamp bytes at the row boundary`() {
    val phase = """{"contract_version":"$FEATURE_TASK_RUNTIME_PERSISTENCE_CONTRACT_VERSION","record_kind":"private_phase_record","phase_id":"plan","status":"running","attempt_count":1,"started_at":"2026-06-02 10:00:00","first_started_at":"2026-06-02T12:00:00.000+02:00","resolved_agent_id":"codex","execution_origin":"agent-executed"}"""
    val ledger = """{"action":"start","sequence_number":0,"timestamp":"2026-06-02T10:00:00.000Z","phase_id":"plan","attempt_count":1,"execution_origin":"agent-executed"}"""
    val progress = """{"contract_version":"$GOAL_PROGRESS_EVENT_CONTRACT_VERSION","event_kind":"phase_started","workflow_id":"wf-1","workflow_phase":"plan","process_alive":true,"sequence_number":0,"timestamp":"2026-06-02T12:00:00.000+02:00"}"""
    val source = row(artifactsJson = """{"feature_task_runtime_phase_records":{"plan":$phase},"feature_task_runtime_phase_ledger":[$ledger],"goal_progress_latest_event":$progress}""")
    ...
    val progressEvent = assertNotNull(JsonCodec.anyToStringAnyMap(JsonCodec.parseValue(progress)))
      .decodeDeclaredGoalProgressEvent("test")
    val updated = source.toSnapshot().copy(artifacts = DurableWorkflowArtifacts.fromMap(linkedMapOf(
  ...
  fun `attempt ledger encoding retains legacy timestamp text through a row rewrite`() {
    val source = row(artifactsJson = """{"goal_attempt_ledger":[{"action":"resume","sequence_number":1,"timestamp":"2026-06-02 10:00:00"}]}""")
    val entry = GoalAttemptLedgerEntry(GoalAttemptLedgerAction.RESUME, 1, "2026-06-02 10:00:00")
    val snapshot = source.toSnapshot().copy(artifacts = DurableWorkflowArtifacts.fromMap(
  ...
  fun `goal observability timestamp spellings survive an unrelated artifact update`() {
    val source =
      row(
        artifactsJson =
          """{"goal_observability_latest_event":{"timestamp":"2026-06-02T12:00:00.000+02:00","sequence_number":1},"goal_observability_run_history":[{"timestamp":"2026-06-02 10:00:00","sequence_number":0}]}""",
      )
    val updated =
    ...
    assertEquals(
      """{"goal_observability_latest_event":{"timestamp":"2026-06-02T12:00:00.000+02:00","sequence_number":1},"goal_observability_run_history":[{"timestamp":"2026-06-02 10:00:00","sequence_number":0}],"unrelated":true}""",
