...
import skillbill.engine.featuretask.NON_FILE_MUTATING_PHASES
...
import skillbill.engine.featuretask.reconcileCheckpointPathInventory
...
class FeatureTaskRuntimeRunnerAgentResumeTest {
  ...
  fun `resume restarts from last incomplete phase and restores upstream outputs`() {
    ...
    val auditPrompt = harness.launcher.requests.map { requireNotNull(it.skillRunRequest.promptOverride) }
      .single { phaseIdFromPrompt(it) == "audit" }
    assertFalse(briefings.containsKey("audit"))
    assertContains(auditPrompt, "Fixture plan prose for downstream implement and audit.")
    assertContains(auditPrompt, "Fixture implement prose for downstream audit.")
    assertFalse(auditPrompt.contains("Phase produced a validated output."))

    assertFalse(auditPrompt.contains("from: review"))
    val reviewBriefing = requireNotNull(briefings["review"]) { "review briefing must be persisted" }
    assertFalse(reviewBriefing.briefingText.contains("clearance_status"))
  ...
  fun `absent-gate validate without gate_run_count completes on first attempt`() {
    val harness = runnerHarness(
      RuntimeHarnessConfig(
        agentAssignment = phasePerAgentAssignment(),
        validationGatePlatformManifests = emptyList(),
        launcher = RuntimeRecordingLauncher { request ->
          val phaseId = phaseIdFromPrompt(requireNotNull(request.skillRunRequest.promptOverride))
          facts(
            if (phaseId == "validate") {
...
class FeatureTaskRuntimeLifecycleTelemetryRunnerTest {
  ...
  fun `runtime lifecycle telemetry uses the bound database session`() {
    ...
    assertIs<FeatureTaskRuntimeRunReport.Completed>(report)
    assertTrue(harness.database.transactionCount > 0)
...
class FeatureTaskRuntimeRunnerPersistenceTest {
  ...
  fun `persists per-phase briefing durably with run-invariants upstream and review diff`() {
    ...
    val briefings = harness.recorder.loadPhaseBriefings(WORKFLOW_ID).orEmpty()
    assertEquals(COMPLETED_PHASES_CLEAN_RUN.toSet() - "audit", briefings.keys)
    ...
    assertFalse(briefings.getValue("review").briefingText.contains("clearance_status"))
  ...
  fun `partial resume launches review with durable small size and current unit scope`() {
    val harness = runnerHarness(smallRuntimeConfig().copy(agentAssignment = phasePerAgentAssignment()))
...
class FeatureTaskRuntimeGoalContinuationReviewPrepTest {
  ...
  fun `non-scope failure inside review preparation surfaces its own cause not the fixed scope sentence`() {
    assertNonScopeReviewPrepFailureSurfacesEvidenceStoreCause()
  }
...
class FeatureTaskRuntimeGoalContinuationStaleReviewTest {
  ...
  fun `cappedReviewIsStale ignores an unreachable remediation base and keeps an unchanged immutable digest`() {
    assertCappedReviewStaleIgnoresUnreachableRemediationBase()
  }

  @Test
  fun `cappedReviewIsStale still reopens when the immutable base digest changed despite unreachable remediation`() {
    assertCappedReviewStaleReopensWhenImmutableDigestChanged()
  }
  ...
  fun `crash reconciliation preserves a changes requested disposition without structured findings`() {
    ...
    harness.seedPhase("plan", "completed", 1, phaseAgent("plan"), PLAN_OUTPUT)
    harness.seedPhase("implement", "completed", 1, phaseAgent("implement"), IMPLEMENT_OUTPUT)
    harness.seedPhase("audit", "completed", 1, phaseAgent("audit"), validJsonOutput("audit"))
    harness.seedPhase(
      "review",
      "completed",
      1,
...
class FeatureTaskRuntimeCheckpointScopeTest {
  ...
  fun `goal-child audit checkpoint scopes owned paths to the child's own base and baseline inventory`() {
    ...
    val auditPrompt = harness.launcher.requests.map { requireNotNull(it.skillRunRequest.promptOverride) }
      .single { phaseIdFromPrompt(it) == "audit" }
    assertContains(auditPrompt, "base_ref: ${"0".repeat(40)}")
    assertContains(auditPrompt, "- runtime-domain/Child.kt")
    assertContains(auditPrompt, "- runtime-domain/Committed.kt")
    assertContains(auditPrompt, "- runtime-domain/Remediation.kt")
    assertContains(auditPrompt, "- runtime-domain/Renamed.kt")
    assertFalse(
      auditPrompt.contains("- $SPEC_REFERENCE"),
      "the local feature spec is workflow input, not an audit or commit path",
    )
    assertFalse(
      auditPrompt.contains("spec_subtask_9_sibling"),
      "a sibling subtask's baseline path must not enter the goal-child audit projection",
    )
    assertFalse(
      auditPrompt.contains(".feature-specs/SKILL-137/sibling"),
  ...
  fun `an unmeasurable owned-path read blocks audit instead of rendering an empty scope`() {
    ...
    git.ownedPathsResult = WorkflowGitOperationResult.Failed(value = "")
    val harness = runnerHarness(
      RuntimeHarnessConfig(
        branchSetup = BranchSetupTestConfig(gitOperations = git),
      ).copy(agentAssignment = phasePerAgentAssignment()),
...
class FeatureTaskRuntimeReviewFixLoopTest {
  ...
  fun `m1 cap exhaustion advances to validate without re-review`() {
    val harness = runnerHarness(reviewFixRuntimeConfig(2).copy(launcher = reviewFixLauncher(convergeOnReview = 2)))
  ...
  fun `m1 an unresolved Blocker advances to validate after one fix round`() {
    val harness = runnerHarness(reviewFixRuntimeConfig(12).copy(launcher = reviewFixLauncher(convergeOnReview = 12)))
...
class FeatureTaskRuntimeBranchSetupRunnerTest {
  ...
  fun `cannot establish a feature branch blocks loudly and launches no file-mutating phase`() {
    val git = RecordingWorkflowGitOperations(
      ...
      checkoutResult = WorkflowGitOperationResult.Failed(error = "checkout exploded"),
    )
    val harness = runnerHarness(conventionRuntimeConfig(git))
  ...
  fun `unreadable current branch blocks loudly and launches no file-mutating phase`() {
    val git = RecordingWorkflowGitOperations(
      currentBranchResult = WorkflowGitOperationResult.Failed(error = "git HEAD unreadable"),
    )
    val harness = runnerHarness(conventionRuntimeConfig(git))
  ...
  fun `resume blocks loudly when persisted branch existence cannot be verified`() {
    ...
    val git = RecordingWorkflowGitOperations(
      currentBranchValue = "main",
      branchExistsResult = WorkflowGitOperationResult.Failed(error = "rev-parse exploded"),
    )
    val harness = runnerHarness(conventionRuntimeConfig(git))
    harness.seedResolvedBranch(persistedBranch, baseBranch = "main", created = true)
    harness.seedPhase("plan", "completed", 1, INVOKED_AGENT, PLAN_OUTPUT)
  ...
  fun `no file-mutating phase launches while on the default branch`() {
    val git = RecordingWorkflowGitOperations(
      currentBranchValue = "main",
      checkoutResult = WorkflowGitOperationResult.Failed(error = "denied"),
    )
    val harness = runnerHarness(conventionRuntimeConfig(git))
...
class FeatureTaskRuntimeReconcileOnResumeTest {
  ...
  fun `dirty tree checkpoint that fails to stage blocks loudly and never commits`() {
    val git = RecordingWorkflowGitOperations(currentBranchValue = "feat/existing-runtime-branch")
    git.stagePathsResult = WorkflowGitOperationResult.Failed(error = "stage failed")
    val harness = runnerHarness(
      RuntimeHarnessConfig(
        branchSetup = BranchSetupTestConfig(gitOperations = git),
        reviewDriver = reviewFixDriver(2),
  ...
  fun `a failed checkpoint commit restores the pre-checkpoint index and reports the restore outcome`() {
    ...
    git.createCommitResult = WorkflowGitOperationResult.Failed(error = "commit failed")
    val harness = checkpointRunHarness(git)
  ...
  fun `a restore failure is reported in the checkpoint block reason rather than swallowed`() {
    val git = checkpointGit(ownedPaths = listOf("src/Owned.kt"))
    git.createCommitResult = WorkflowGitOperationResult.Failed(error = "commit failed")
    git.restoreIndexStateResult = WorkflowGitOperationResult.Failed(error = "restore failed")
    val harness = checkpointRunHarness(git)
  ...
  fun `a failed pre-amend ref write blocks the checkpoint and leaves HEAD unchanged`() {
    val git = checkpointGit(ownedPaths = listOf("src/Owned.kt"))
    git.updateCheckpointRefResult = WorkflowGitOperationResult.Failed(error = "ref write refused")
    val harness = checkpointRunHarness(git)
  ...
  fun `an amend whose checkpoint ref occupancy cannot be determined is refused before HEAD is rewritten`() {
    val git = checkpointGit(ownedPaths = listOf("src/Owned.kt"))
    git.resolveCheckpointRefResult = WorkflowGitOperationResult.Failed(error = "ref lookup failed")
    val harness = checkpointRunHarness(git)
...
class FeatureTaskRuntimeCheckpointHistoryOnResumeTest {
  ...
  fun `dirty tree with a protected resolved branch never checkpoints`() {
    val git = RecordingWorkflowGitOperations(currentBranchValue = "main")
    git.worktreeStatusValue = " M src/Foo.kt"
    val harness = runnerHarness(conventionRuntimeConfig(git).copy(agentAssignment = phasePerAgentAssignment()))
    harness.seedResolvedBranch("main", baseBranch = "main", created = false)
...
class FeatureTaskRuntimeProducerEvidenceIdentityTest {
  ...
  fun `cross-agent retain at review generation 0 attempt 2 continues without Conflict`() {
    ...
    assertContentEquals(
      claudePayload,
      harness.io.database.producerEvidenceAt(ProducerEvidenceKey(WORKFLOW_ID, "review", 0, 2, "claude", 0))?.payload,
    )
    assertContentEquals(
      cursorPayload,
      harness.io.database.producerEvidenceAt(ProducerEvidenceKey(WORKFLOW_ID, "review", 0, 2, "cursor", 0))?.payload,
    )
  }
