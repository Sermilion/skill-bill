...
class GoalRunnerHardResetOrphanTrapTest {
  ...
  fun `review baseline block for subtask 2 includes one hard reset command`() {
    val manifest = skillbill.engine.manifest(2).copy(
      issueKey = issueKey,
      subtasks = listOf(
        skillbill.engine.manifest(2).subtasks[0].copy(status = "complete", commitSha = parentSha),
        skillbill.engine.manifest(2).subtasks[1].copy(status = "pending"),
      ),
  ...
  private fun manifest() = skillbill.engine.manifest(1).copy(
    issueKey = issueKey,
    status = "in_progress",
    featureBranch = branch,
    currentSubtaskIntent = CurrentSubtaskIntent(subtaskId = 1, action = "resume"),
    subtasks = listOf(
      skillbill.engine.manifest(1).subtasks.single().copy(
        status = "in_progress",
