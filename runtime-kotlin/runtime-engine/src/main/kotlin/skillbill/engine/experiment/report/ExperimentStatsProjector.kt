...
object ExperimentStatsProjector {
  fun project(reports: List<Map<String, Any?>>): Map<String, Any?> =
    linkedMapOf(
  ...
  private fun aggregateMetrics(reports: List<Map<String, Any?>>): List<Map<String, Any?>> =
    reports
      .flatMap { report ->
      ...
      .map { (metricId, comparisons) ->
        val savings =
          comparisons.mapNotNull { comparison ->
        ...
        linkedMapOf<String, Any?>(
          ...
          ExperimentStatsPayloadKeys.AVERAGE_PERCENT_SAVINGS to
            comparisons.mapNotNull { comparison ->
              (
  ...
  private fun totalSpend(reports: List<Map<String, Any?>>): Map<String, Any?> {
    ...
    return mapOf(
      ...
      ExperimentReportPayloadKeys.AMOUNT to
        values.sumOf {
  ...
  private fun quality(reports: List<Map<String, Any?>>): Map<String, Any?> {
    ...
    return mapOf(
      ExperimentStatsPayloadKeys.LABELLED_CRITERIA to
        arms.sumOf {
          (it[ExperimentReportPayloadKeys.LABELLED_CRITERIA] as? Number)?.toInt() ?: 0
        },
      ExperimentStatsPayloadKeys.TOTAL_CRITERIA to
        arms.sumOf {
          (it[ExperimentReportPayloadKeys.TOTAL_CRITERIA] as? Number)?.toInt() ?: 0
        },
      ExperimentStatsPayloadKeys.PRECISION_AVAILABLE_PAIRS to
        arms.count {
          it[ExperimentReportPayloadKeys.PRECISION_AVAILABLE] == true
        },
      ExperimentStatsPayloadKeys.RESTRICTED_BASELINE_PAIRS to
        arms.count {
