...
object ExperimentTelemetryPayloadBuilder {
  fun build(
    ...
    val payload = linkedMapOf<String, Any?>(
      ExperimentTelemetryPayloadKeys.PAIR_ID to if (consent == ExperimentTelemetryConsent.FULL) {
        pairId
      } else {
        hash(
          pairId,
  ...
  private fun hash(value: String): String = MessageDigest.getInstance("SHA-256").digest(value.encodeToByteArray())
