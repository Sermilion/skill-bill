...
object FeatureTaskRuntimeRunLoopRepairReceipt {
  ...
  internal fun recordRepairReceiptWriteFailure(request: FeatureTaskRuntimeRunRequest, diagnostics: RuntimeDiagnostics, error: Throwable){
    diagnostics.warning(
  ...
  internal fun settleAndPersistImplementFixRepairReceipt(
    request: FeatureTaskRuntimeRunRequest, state: FeatureTaskRuntimeRunState, recorder: FeatureTaskRuntimePhaseRecorder, observability: FeatureTaskRuntimeRunObservability,
    goalContinuationRecorder: FeatureTaskRuntimeGoalContinuationRecorder,
    diagnostics: RuntimeDiagnostics,
  ...
  internal fun implementFixRepairReceiptSettlement(
    ...
    return when (
      ...
      is FeatureTaskRuntimeRepairReceiptValid -> settledRepairReceipt(
        request, state, recorder,
        goalContinuationRecorder,
        diagnostics,
        parsed.receipt,
  ...
  internal fun settledRepairReceipt(
    ...
    ?: persistImplementFixRepairReceipt(request, state, goalContinuationRecorder, diagnostics, receipt)?.let { reason -> RepairReceiptSettlement.writeFailed(reason) }
    ?: RepairReceiptSettlement.None

  internal fun refutedCarriedFindingIds(request: FeatureTaskRuntimeRunRequest, recorder: FeatureTaskRuntimePhaseRecorder, diagnostics: RuntimeDiagnostics, reviewState: GoalSubtaskReviewState): Set<String> {
  ...
  internal fun repairReceiptAnchor(request: FeatureTaskRuntimeRunRequest, diagnostics: RuntimeDiagnostics, reviewState: GoalSubtaskReviewState): RepairReceiptAnchor? {
    ...
    if (baseSha == null || roundNumber == null) {
      recordRepairReceiptDegradation(request, diagnostics, if (baseSha == null) {
  ...
  internal fun recordRepairReceiptDegradation(request: FeatureTaskRuntimeRunRequest, diagnostics: RuntimeDiagnostics, reason: String){
    runCatching {
      diagnostics.warning(
  ...
  internal fun blockRemediationBaseSha(request: FeatureTaskRuntimeRunRequest, state: FeatureTaskRuntimeRunState, session: FeatureTaskRuntimeRunLoopSession, precedingPhaseId: String, error: String): Boolean {
    FeatureTaskRuntimeRunLoopPlanningBranch.blockAt(request, state, session, precedingPhaseId, "Feature-task-runtime could not record the pre-fix remediation base sha before re-entering " +
  ...
  private fun blockCheckpointAfterIndexMutation(request: FeatureTaskRuntimeRunRequest, state: FeatureTaskRuntimeRunState, session: FeatureTaskRuntimeRunLoopSession, phaseGates: FeatureTaskRuntimePhaseGates, args: CommitCheckpointArgs, error: String, indexSnapshot: String): Boolean = FeatureTaskRuntimeRunLoopCheckpoint.blockCheckpoint(
    request, state, session,
    args.precedingPhaseId,
    args.branch,
    FeatureTaskRuntimeRunLoopCheckpoint.withIndexRestoreOutcome(request, phaseGates, error, args.ownedPaths, indexSnapshot),
    args.blockedReason,
  )

  internal fun commitCheckpoint(request: FeatureTaskRuntimeRunRequest, state: FeatureTaskRuntimeRunState, recorder: FeatureTaskRuntimePhaseRecorder, session: FeatureTaskRuntimeRunLoopSession, diagnostics: RuntimeDiagnostics, phaseGates: FeatureTaskRuntimePhaseGates, args: CommitCheckpointArgs): Boolean {
    ...
    if (snapshot !is WorkflowGitOperationResult.Ok) {
      return FeatureTaskRuntimeRunLoopCheckpoint.blockCheckpoint(request, state, session, precedingPhaseId, branch, snapshot.error, blockedReason)
    }
    ...
    if (staged !is WorkflowGitOperationResult.Ok) {
      return blockCheckpointAfterIndexMutation(request, state, session, phaseGates, args, staged.error, snapshot.value.orEmpty())
    }
    ...
    val commit = FeatureTaskRuntimeRunLoopCheckpoint.writeSubtaskCommit(request, recorder, diagnostics, phaseGates, branch, message, subtaskIdentity)
    if (commit !is WorkflowGitOperationResult.Ok) {
      return blockCheckpointAfterIndexMutation(request, state, session, phaseGates, args, commit.error, snapshot.value.orEmpty())
    }
    return FeatureTaskRuntimeRunLoopCheckpoint.recordCheckpointIdentity(request, state, recorder, session, RecordCheckpointIdentityArgs(
