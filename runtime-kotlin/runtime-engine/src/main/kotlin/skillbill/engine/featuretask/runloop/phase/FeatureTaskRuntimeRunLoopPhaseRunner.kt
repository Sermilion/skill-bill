...
import skillbill.engine.featuretask.runloop.core.GoalReviewRunReady
import skillbill.engine.featuretask.runloop.core.LEGACY_PLANNING_PROJECTION_LAUNCH_SEAM_REJECTION
import skillbill.engine.featuretask.runloop.core.MissingCarriedForwardGoalReviewResultException
...
import skillbill.engine.featuretask.runloop.core.RuntimeOwnedReviewPreparationArgs
...
object FeatureTaskRuntimeRunLoopPhaseRunner {
  ...
  internal fun prepareStandaloneReviewRun(context: GoalReviewContext): GoalReviewRunPreparation {
    val resolved =
      context.recorder.loadResolvedBranch(context.run.request.workflowId)
    ...
    val result =
      context.phaseGates.gitOperations.buildGoalSubtaskReviewInput(
  ...
  internal fun reserveGoalReviewRun(context: GoalReviewContext): GoalReviewRunPreparation =
    runCatching {
  ...
  internal fun buildGoalReviewRun(context: GoalReviewContext): GoalReviewRunPreparation =
    runCatching {
      ...
      requireNotNull(context.goalContinuationRecorder).buildGoalReviewInput(
        ...
        scope =
          FeatureTaskRuntimeGoalContinuationRecorder.GoalReviewInputScope(
  ...
  internal fun loadCarriedForwardGoalReviewOutput(
    ...
    val output =
      goalContinuationRecorder.lastGoalReviewResult(run.request.workflowId)
  ...
  internal fun carriedForwardMissingReviewBlock(
    ...
    return BlockAndPersistArgs(
      ...
      reason =
        "Goal-subtask review pass budget is exhausted but its durable raw " +
