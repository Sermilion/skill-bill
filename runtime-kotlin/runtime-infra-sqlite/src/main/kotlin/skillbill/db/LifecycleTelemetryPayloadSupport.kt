...
fun featureTaskRuntimeFinishedPayload(row: Map<String, Any?>, level: String): Map<String, Any?> =
  featureTaskRuntimeStartedPayload(row, level).toMutableMap().apply {
    ...
    put("duration_seconds", durationSeconds(row))
    put("parallel_review_requested", row.booleanFromInt("parallel_review_requested"))
    put("default_review_agent_id", row.stringOrEmpty("default_review_agent_id"))
    put("alternative_review_agent_id", row.stringOrEmpty("alternative_review_agent_id"))
    put("review_lane_count", row.intOrZero("review_lane_count").coerceAtLeast(1))
    put("review_lane_statuses", JsonSupport.parseArrayOrEmpty(row.stringOrEmpty("review_lane_statuses")))
    put("merged_review_finding_count", row.intOrZero("merged_review_finding_count"))
    put("accepted_review_finding_count", row.intOrZero("accepted_review_finding_count"))
    put("rejected_review_finding_count", row.intOrZero("rejected_review_finding_count"))
    put("unresolved_review_finding_count", row.intOrZero("unresolved_review_finding_count"))
    if (level == "full") {
