...
internal fun severityViolations(file: Path): List<ReviewSkillStructureViolation> =
  severityRatings(
...
internal fun severityRatings(content: String): Set<String> =
  buildSet {
...
internal fun hasBespokeFocuses(
  focuses: List<Pair<String, String>>,
  metadataSize: Int,
  packLabel: String,
): Boolean {
...
internal fun hasCanonicalSeverityCloser(
  area: String?,
  content: String,
): Boolean {
...
internal fun declaredAreasForContent(file: Path): Set<String> =
  manifest(file.parent.parent.parent)
    ?.let(::declaredAreas)
    ?.toSet()
    .orEmpty()
