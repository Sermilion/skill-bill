...
internal fun previewCreatedFiles(plan: ScaffoldPlan): List<Path> =
  when (plan.kind) {
    SKILL_KIND_PLATFORM_PACK -> previewPlatformPackCreatedFiles(plan) + previewSubagentStubFiles(plan)
    SKILL_KIND_ADD_ON ->
      if (plan.isExternalAddon() && !Files.exists(plan.externalAddonManifestPath())) {
...
internal fun previewManifestEdits(
  plan: ScaffoldPlan,
  repoRoot: Path,
): List<Path> =
  when (plan.kind) {
...
internal fun previewManifestPreviews(
  plan: ScaffoldPlan,
  repoRoot: Path,
): Map<Path, String> =
  when (plan.kind) {
    ...
    SKILL_KIND_ADD_ON -> {
      if (plan.addonConsumerSkillDirs.isEmpty()) {
        ...
        mapOf(
          manifestPath to
            renderExternalAddonManifestRegistration(
              text = current,
