...
internal fun parseAddonUsageEntry(
...
internal fun requireAddonUsageEntry(
  context: AddonUsageParseContext,
  index: Int,
  raw: Any?,
): Map<*, *> =
...
internal fun parseAddonActivation(
  ...
  val anyOfAllContent =
    (rawActivation["any_of_all_content"] as? List<*>)?.mapIndexed { groupIndex, group ->
      parseStringList(
        context.slug,
        group,
...
internal fun requirePackOwnedAddonPointer(
  ...
  val pointer =
    context.pointersForDir.firstOrNull { spec -> spec.name == pointerName }
      ?: invalidManifestSchema(
  ...
  if (context.fieldName == "feature_addon_usage") {
    val repoRoot =
      context.packRoot.parent?.parent
        ?: invalidManifestSchema(
