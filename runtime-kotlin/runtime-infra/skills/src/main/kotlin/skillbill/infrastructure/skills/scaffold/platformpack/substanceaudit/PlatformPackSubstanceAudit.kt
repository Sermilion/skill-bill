...
internal object PlatformPackSubstanceAudit {
  fun audit(
    repoRoot: Path,
    policy: SubstancePolicy = SubstancePolicy(),
  ): PlatformPackSubstanceReport = PlatformPackSubstanceAuditCore.audit(repoRoot, policy)

  fun normalize(
    text: String,
    names: Collection<String> = emptyList(),
  ): List<String> = PlatformPackSubstanceAuditCore.normalize(text, names.toList())
