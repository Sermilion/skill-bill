...
object ReadmeCatalogEdits {
  private fun catalogRowPattern(skillName: String): Regex =
    Regex(
      "^\\|\\s*[`\"]?/?${Regex.escape(skillName)}[`\"]?\\s*\\|[^\\n]*\\n",
      RegexOption.MULTILINE,
    )

  private val SECTION_COUNT_PATTERN =
    Regex(
      "^(### Canonical Skills \\()(\\d+)( skills?\\))\\s*$",
      RegexOption.MULTILINE,
    )

  fun removeCatalogRow(
    readmePath: Path,
    skillName: String,
  ): ReadmeEditOutcome {
    ...
    val match =
      pattern.find(original) ?: return ReadmeEditOutcome.LandmarksMissing(
  ...
  fun decrementSectionCount(readmePath: Path): ReadmeEditOutcome {
    val original = readmePath.toFile().readText()
    val match =
      SECTION_COUNT_PATTERN.find(original) ?: return ReadmeEditOutcome.LandmarksMissing(
