package skillbill.infrastructure.skills.nativeagent.platformpack

import java.nio.file.Path

internal data class NativeAgentPointerSpec(
  val skillRelativeDir: String,
  val name: String,
  val target: String,
)

internal data class NativeAgentGovernedAddonActivation(
...
internal data class NativeAgentGovernedAddonSelection(
  val slug: String,
...
internal data class NativeAgentGovernedAddonUsage(
  val skillRelativeDir: String,
  val addons: List<NativeAgentGovernedAddonSelection>,
)

internal data class NativeAgentDeclaredFiles(
  val baseline: Path?,
  val areas: Map<String, Path>,
)

internal data class NativeAgentPlatformPack(
  val slug: String,
  val packRoot: Path,
...
internal interface NativeAgentPlatformPackLoader {
  fun loadPlatformPack(packRoot: Path, additionalPackRoots: List<Path> = emptyList()): NativeAgentPlatformPack
