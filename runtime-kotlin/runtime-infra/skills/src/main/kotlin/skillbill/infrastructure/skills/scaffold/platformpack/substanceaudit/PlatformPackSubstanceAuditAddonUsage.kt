...
internal fun correspondingPairs(files: List<AuthoredFile>): List<SimilarityPair> =
  files.groupBy {
    roleKey(it)
...
internal fun roleKey(file: AuthoredFile): String =
  when (file.role) {
...
internal fun fraction(
  numerator: Int,
  denominator: Int,
): Fraction = if (denominator == 0) Fraction(0, 1) else Fraction(numerator, denominator)
