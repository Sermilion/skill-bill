...
internal fun parseValidationGate(
  manifest: Map<*, *>,
  slug: String,
): ValidationGateDeclaration? {
  val raw = manifest["validation_gate"] ?: return null
  val gate =
    raw as? Map<*, *> ?: invalidValidationGateDeclaration(
  ...
  val cacheBypassingCollectAllFullGateCommand =
    requireGateArgv(
      gate,
      slug,
...
internal fun parseSuppressionMarkers(
  gate: Map<*, *>,
  slug: String,
): List<String> {
  ...
  val values =
    raw as? List<*> ?: invalidValidationGateDeclaration(
...
internal fun parseValidationGateFindings(
  gate: Map<*, *>,
  slug: String,
): ValidationGateFindingsLocator {
  val raw =
    gate[ReviewVerificationSignalKeys.REVIEW_FINDINGS] ?: invalidValidationGateDeclaration(
  ...
  val globs =
    globsRaw.mapIndexed { index, value ->
      (value as? String)?.trim()?.takeIf(String::isNotEmpty)
        ?: invalidValidationGateDeclaration(
...
internal fun parseCompilerDiagnosticsLocator(
  ...
  val raw =
    findings["compiler_diagnostics"] ?: invalidValidationGateDeclaration(
...
internal fun parseExecutedWorkSignal(
  raw: Any?,
  slug: String,
