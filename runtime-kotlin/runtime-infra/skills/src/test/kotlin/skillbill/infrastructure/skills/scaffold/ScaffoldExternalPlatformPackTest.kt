...
class ScaffoldExternalPlatformPackTest {
  @Test
  fun `external create writes the pack outside the repo and registers it`() = withIsolatedUserHome {
    val repo = seedRepo()
    val destination = Path.of(System.getProperty("user.home")).resolve("packs/acme")
    val result = scaffoldExternal(
  ...
  @Test
  fun `external dry-run writes no pack files and no config`() = withIsolatedUserHome {
    val repo = seedRepo()
    val destination = Path.of(System.getProperty("user.home")).resolve("packs/acme")
    val result = scaffoldExternal(
      payload(repo, destination.toString(), "create"),
  ...
  @Test
  fun `create against an existing destination leaves that directory and writes no config`() = withIsolatedUserHome {
    val repo = seedRepo()
    val destination = Path.of(System.getProperty("user.home")).resolve("packs/acme")
  ...
  @Test
  fun `register existing pack records the canonical path and does not rewrite platform yaml`() = withIsolatedUserHome {
    val repo = seedRepo()
    val destination = Path.of(System.getProperty("user.home")).resolve("packs/acme")
  ...
  @Test
  fun `validation failure after create removes the new tree and the added config entry`() = withIsolatedUserHome {
    val repo = seedRepo()
  ...
  @Test
  fun `register existing pack ignores a sibling directory that is not a pack`() = withIsolatedUserHome {
    val repo = seedRepo()
    val destination = Path.of(System.getProperty("user.home")).resolve("packs/acme")
    writeConformingPack(destination)
  ...
  private fun externalRuntime() = ScaffoldRuntimeContext(
  ...
  private fun scaffoldExternal(payload: Map<String, Any?>, dryRun: Boolean, adapters: ScaffoldAdapterSeams) =
    scaffoldWithAdapters(payload, dryRun, adapters, runtime = externalRuntime())

  private fun seams(validate: (ScaffoldPlan, Path) -> Unit): ScaffoldAdapterSeams = ScaffoldAdapterSeams(
