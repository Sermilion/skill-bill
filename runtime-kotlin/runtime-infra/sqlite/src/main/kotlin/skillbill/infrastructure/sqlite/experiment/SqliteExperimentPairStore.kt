...
internal class SqliteExperimentPairStore(
  ...
  override fun loadPairPayload(pairId: String): Pair<String, Map<String, Any?>>? = connection.prepareStatement(
    """
  ...
  private fun loadArmOutcomes(pairId: String): List<Map<String, Any?>> = connection.prepareStatement(
    """
    ...
    statement.executeQuery().use { rows ->
      buildList {
        while (rows.next()) {
          val persisted = rows.getString("outcome_json")
            ?.let(::jsonObjectMap)
            ?.toMutableMap()
            ?: mutableMapOf()
  ...
  private fun loadObservationLedger(pairId: String): List<Map<String, Any?>> = connection.prepareStatement(
    """
    ...
    statement.executeQuery().use { rows ->
      buildList {
        while (rows.next()) {
          val measurements = jsonObjectMap(rows.getString("measurements_json"))
          ...
          add(
            mapOf(
              ...
              ExperimentObservationPayloadKeys.EVENT_IDENTITY to jsonObjectMap(
  ...
  private fun persistArmOutcomes(pairId: String, payload: Map<String, Any?>) {
  ...
  override fun insertObservationIfAbsent(observation: ExperimentObservationImport): Boolean {
    val payloadMap = JsonCodec.parseObjectOrNull(observation.payloadJson)?.let { element ->
      JsonCodec.anyToStringAnyMap(JsonCodec.jsonElementToValue(element))
    }
    val measurementsJson = JsonCodec.mapToJsonString(
      mapOf(
  ...
  override fun deletePairsForWorkflowIds(workflowIds: List<String>) {
    ...
    val pairIds = connection.prepareStatement(
    ...
    val liveLease = connection.prepareStatement(
  ...
  override fun loadReport(pairId: String): Map<String, Any?>? = connection.prepareStatement(
  ...
  override fun listReports(): List<Map<String, Any?>> = connection.prepareStatement(
