...
import skillbill.infrastructure.sqlite.goalrunner.control.acceptance
import skillbill.infrastructure.sqlite.goalrunner.control.acquireExecutionLease
import skillbill.infrastructure.sqlite.goalrunner.control.authorizeSubtaskLaunch
import skillbill.infrastructure.sqlite.goalrunner.control.bindRepositoryIdentity
import skillbill.infrastructure.sqlite.goalrunner.control.clearRunnerInterruptedPause
import skillbill.infrastructure.sqlite.goalrunner.control.codeReviewMode
...
import skillbill.infrastructure.sqlite.goalrunner.control.mode
import skillbill.infrastructure.sqlite.goalrunner.control.pauseAtBoundary
import skillbill.infrastructure.sqlite.goalrunner.control.pauseNow
import skillbill.infrastructure.sqlite.goalrunner.control.persistControlState
...
import skillbill.infrastructure.sqlite.goalrunner.control.policy
...
import skillbill.infrastructure.sqlite.goalrunner.control.saveCompletedSubtaskAtBoundary
...
import skillbill.infrastructure.sqlite.goalrunner.outcome.blockedReason
import skillbill.infrastructure.sqlite.goalrunner.outcome.executeScopedReplan
import skillbill.infrastructure.sqlite.goalrunner.outcome.issueKey
import skillbill.infrastructure.sqlite.goalrunner.outcome.projectionArtifactsJson
import skillbill.infrastructure.sqlite.goalrunner.outcome.root
import skillbill.infrastructure.sqlite.goalrunner.outcome.saveInTransaction
...
import skillbill.infrastructure.sqlite.goalrunner.outcome.workflowSnapshotValidator
...
class WorkflowGoalRunnerManifestStore @Inject constructor(
  ...
  override fun requestPauseByIssueKey(issueKey: String, repoRoot: Path?): GoalRunnerPausePersistenceResult? =
    controls.requestPauseByIssueKey(issueKey, repoRoot)
  ...
  override fun heartbeatExecutionLease(parentWorkflowId: String, lease: GoalRunnerExecutionLease): Boolean =
    controls.heartbeatExecutionLease(parentWorkflowId, lease)

  override fun releaseExecutionLease(parentWorkflowId: String, ownerToken: String, generation: Long): Boolean =
    controls.releaseExecutionLease(parentWorkflowId, ownerToken, generation)
  ...
  override fun releaseExecutionLeaseIfExpired(
    ...
    generation: Long,
    nowInstant: String,
  ): Boolean = controls.releaseExecutionLeaseIfExpired(
    parentWorkflowId,
    ownerToken,
    generation,
    nowInstant,
  ...
  override fun bindRepositoryIdentity(parentWorkflowId: String, repositoryIdentity: String): GoalRunnerControlState =
    controls.bindRepositoryIdentity(parentWorkflowId, repositoryIdentity)

  override fun authorizeSubtaskLaunch(state: GoalRunnerManifestState, subtaskId: Int): GoalRunnerLaunchAuthorization =
    controls.authorizeSubtaskLaunch(state, subtaskId)
  ...
  override fun persistStopAfterSubtask(parentWorkflowId: String, subtaskId: Int): GoalRunnerControlState =
    controls.persistStopAfterSubtask(parentWorkflowId, subtaskId)
  ...
  ): GoalPlanningStatusSnapshot? = database.read {
  ...
  override fun saveHardReset(state: GoalRunnerManifestState, preservePlanning: Boolean): GoalRunnerManifestState {
    val saved = database.transaction { unitOfWork ->
  ...
  override fun sharedPreplanPayloadSha256(parentWorkflowId: String): String? = database.read {
