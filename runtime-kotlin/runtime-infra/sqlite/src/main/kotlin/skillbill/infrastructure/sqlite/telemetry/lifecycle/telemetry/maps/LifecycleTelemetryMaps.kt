...
internal fun Map<String, Any?>.intOrZero(name: String): Int = when (val value = this[name]) {
  is Number -> value.toInt()
  is String -> value.toIntOrNull() ?: 0
  else -> 0
}

internal fun Map<String, Any?>.longOrZero(name: String): Long = when (val value = this[name]) {
...
internal fun Map<String, Any?>.nullableInt(name: String): Int? = when (val value = this[name]) {
  is Number -> value.toInt()
