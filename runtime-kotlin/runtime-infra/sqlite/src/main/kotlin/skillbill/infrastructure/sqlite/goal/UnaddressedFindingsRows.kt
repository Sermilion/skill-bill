...
internal fun readUnaddressedFinding(rows: ResultSet): UnaddressedFinding =
  UnaddressedFinding(
    ...
    claimVerdict =
      rows.getString(ReviewFindingPayloadKeys.CLAIM_VERDICT)
        ?.trim()
        ?.takeIf(String::isNotBlank)
        ?.let(ReviewClaimVerdict::fromWire),
    scopeDisposition =
      rows.getString(ReviewFindingPayloadKeys.SCOPE_DISPOSITION)?.trim()?.takeIf(String::isNotBlank)
