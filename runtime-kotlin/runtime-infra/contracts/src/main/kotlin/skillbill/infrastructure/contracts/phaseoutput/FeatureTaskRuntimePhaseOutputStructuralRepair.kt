...
internal object FeatureTaskRuntimePhaseOutputStructuralRepair {
  ...
  fun inspect(
    ...
    val raw =
      when (val exact = StrictPhaseOutputParser.parseDocument(phaseOutputText)) {
  ...
  private fun inspectFailedParse(
    ...
    val extracted =
      if (wholeResponseRepair is FeatureTaskRuntimePhaseOutputStructuralRepairDecision.Accepted) {
        null
      } else {
  ...
  private fun selectEmbeddedDocument(
    text: String,
    sourceLabel: String,
  ): EmbeddedDocument? {
    ...
    val extraCloser =
      unmatchedClosingOutsideOffset(text, selected.sourceStart, selected.sourceEnd)
        ?: return selected
    val evidence =
      selected.evidence ?: FeatureTaskRuntimePhaseOutputRepairEvidence(
  ...
  private fun unmatchedClosingOutsideOffset(
    text: String,
    sourceStart: Int,
    sourceEnd: Int,
  ): Int? {
    ...
    val outsideOffset =
      StructuralRepairSyntax.scanDelimiters(maskCodeQuoting(outside))
  ...
  private fun embeddedCandidates(text: String): List<TextCandidate> =
    buildList {
