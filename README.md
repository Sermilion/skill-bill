...
- **Durable state**: `feature_implement_workflow_open` mints a `workflow_id`; every phase boundary writes via `feature_implement_workflow_update` and gets back a compact acknowledgement, not a full snapshot. If a session dies mid-run, `feature_implement_workflow_continue` is the mutating activation path: it re-opens the exact phase and returns a compact continuation payload (resume step, required/available artifact keys, compact current-step artifacts) as the continuation contract — full durable state is fetched only on demand through the read-only `workflow show`. The run survives crashes, compaction, even a host reboot. Same shape exists for `bill-feature-verify` (`feature_verify_workflow_*`).
...
| `/bill-feature-task-runtime` | EXPERIMENTAL runtime-driven feature-task phase loop; not the default path, not auto-routed, and must not destabilize `bill-feature-task` |
