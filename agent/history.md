...
- Release assets are resolved by SUFFIX match (`runtime-cli-*<os>-<arch>.zip`, `runtime-mcp-*…`, `SkillBill-*…{deb,rpm,dmg,msi}`) via the GitHub Releases API, NOT by reconstructing the version — the CI build does not inject the tag as project.version, so filenames may embed `0.1.0-SNAPSHOT`. Default repo `Sermilion/skill-bill`; offline/test seam `SKILL_BILL_RELEASE_DIR` (copy, no network) + `SKILL_BILL_RELEASE_BASE_URL` (curl base, supports file://); `--release <tag>`/`SKILL_BILL_RELEASE_TAG` to pin. reusable
...
Areas: skill_bill/, pyproject.toml, tests/, .mcp.json, runtime-kotlin/runtime-mcp, runtime-kotlin/runtime-cli, runtime-kotlin/runtime-core, skills/bill-quality-check, skills/bill-create-skill, skills/bill-skill-remove, skills/bill-feature-implement/parsing_tolerance.md, orchestration/shell-content-contract, README.md, RELEASING.md, .gitignore
...
- Skill prose and orchestration docs (skills/bill-quality-check, bill-create-skill, bill-skill-remove, bill-feature-implement/parsing_tolerance.md, orchestration/shell-content-contract/PLAYBOOK.md and SCAFFOLD_PAYLOAD.md) re-pointed at Kotlin module names: `ShellContentLoader.loadQualityCheckContent`, `ScaffoldService.scaffold`, `ShellContentContractException` (runtime-contracts), `skillbill.install` (runtime-core).
