...
- Renderer normalizes via `Path.relativize` after `repoRoot.toAbsolutePath().normalize()`, replaces `File.separatorChar` with `/`, strips leading `./`, collapses `//`, emits no trailing newline, and sorts pointers by (skillRelativeDir, name) before write so output is byte-deterministic across machines. reusable
- Path traversal is enforced in TWO layers (defense in depth): the parser (`parsePointerEntry`/`parsePointers` in `ShellContentLoader.kt`) rejects absolute paths and `..` segments in BOTH `target` and `skillRelativeDir` (the original implementation only validated `name`), and the renderer re-asserts `targetFile.startsWith(repoRoot)` plus `pointerFile.startsWith(repoRoot)` plus `pointerFile != targetFile` (self-reference). Symlinked targets that resolve outside repoRoot are rejected via `Files.isRegularFile(..., NOFOLLOW_LINKS)` + `toRealPath()` containment check. reusable
- Pitfall caught in review: renderer initially used `check(...)` which throws `IllegalStateException`, but `runWithUpgradeRollback` only catches `SkillBillRuntimeException`/`IOException`/`IllegalArgumentException`. Switched to `require(...)` so renderer failures during upgrade reach the rollback wrapper. Reusable rule: in any code that lives inside an upgrade-rollback boundary, use `require(...)` (or document the exception type the wrapper accepts) — never `check(...)` unless the wrapper is broadened. reusable
- Pitfall: drift-comparison originally normalized only `\n`, which false-flagged every pointer on Windows CRLF checkouts. Fixed with `trimEnd('\n', '\r')` on both sides. Reusable rule: byte-level file comparisons in cross-platform repos must strip BOTH `\n` and `\r`. reusable
- Migration produced 23 modified pointer files, not the 3 the spec predicted: 10 `..//` typo files (every kotlin specialist's `review-orchestrator.md` plus 2 kmp specialists' — pre-planning surfaced this before implementation) PLUS 13 kmp addon pointer shortenings where `Path.relativize` produces `../../addons/X.md` from the specialist directory instead of the original `../../../../platform-packs/kmp/addons/X.md`. Both forms resolve to the same file; the new form is shorter. Pre-planner-surfaced spec corrections like this should be accepted, not papered over. reusable
- Architecture finding (F-005, fixed via cross-validation rather than refactor): `ScaffoldSupport.ORCHESTRATION_PLAYBOOKS` + `supportingFileTargets()` is a parallel hardcoded mapping at scaffold-time that duplicates each pack's `pointers:` block. Resolved with `validatePointerTargetParity` in `RepoValidationRuntime` that asserts the two maps agree; named `pointers:` as authoritative; left a TODO for the option-1 refactor (drive `supportingFileTargets` from `pointers:` blocks). Reusable rule: when two source-of-truth registries cannot be unified in one PR, add a cross-validator that fails CI if they diverge — buys time without accepting silent drift. reusable
- Validator orphan-detection skips `addons/` and `native-agents/` subtrees structurally (not just by content heuristic) so that future small files in those directories cannot be misclassified as orphan pointers. The content heuristic (size ≤500B + regex `^(\.{1,2}/)+[^\s]+\.md$` + ≤1 newline) remains as a second gate.
- The architecture test forces public DTOs into `runtime-domain/.../model/` packages — `PointerSpec` lives there, while `PointerRendering`/`PointerOperations`/`PointerValidationReport` live in `runtime-core`. ktlint also enforces single-class filenames; that's why the validation report file is `PointerValidationReport.kt` rather than `PointerValidation.kt`. reusable
- New tests: `PointerRoundTripTest` walks all 60 declared pointers across both packs and asserts byte-equality; `PointerRenderingTest` covers self-reference, target-escape-repo, target-is-directory, normalizePointerPath unit (separator+leading-`./`+`//`), forward-slash-only; `PointerManifestParsingTest` covers 12 invalid-entry shapes (missing name/target, blank target, traversal in name/target/skillRelativeDir, duplicate, absolute path, etc.); `PointerOperationsTest` covers idempotent re-run, sorted output, originalBytes sticky across multiple regenerate calls, createdPaths excludes pre-existing, no-op when packs absent; `PointerValidationTest` covers drift/orphan/missing pass+fail plus addons/native-agents-excluded plus multi-issue accumulation+ordering.
Feature flag: N/A
Acceptance criteria: 8/8 implemented
...
- Closed the structural gap that allowed `.agents/skill-overrides.md` mandates to be silently skipped across four 2026-05-02 runs (SKILL-33 FU2/FU3/FU4 + SKILL-34). Triggered by user feedback during SKILL-34 retrospective: "looks like a miss, we need to make sure that skill-override is always respected." The mandate was correctly written into `.agents/skill-overrides.md`'s `## bill-feature-task` section since SKILL-21, but the shared `shell-ceremony.md` rule that pointed skills at the override file was underspecified — it did not say the read is orchestrator-owned, did not require executing action mandates contained within the section, and did not name the lifecycle positions where mandates fire. Skills that delegated the read to a subagent (e.g. `bill-feature-task`'s pre-planning subagent) saw the action mandates paraphrased into a free-form `standards_notes` string and effectively dropped. Personal-graph `session_start` was never called; `write_state`/`write_episode` were never called at finish across any of the four runs. reusable
...
- Pitfall surfaced by user during the run: an early implementation tried to land Step 0 / Step 9b prose in `skills/bill-feature-task/content.md`. That was the wrong layer — `content.md` is the user-editable execution body and orchestration ceremony belongs in `shell-ceremony.md` (or `SKILL.md` for descriptor concerns). The implementation reverted the `content.md` edits and moved enforcement to the ceremony file. reusable
- Predecessor: `[2026-05-02] remove-model-only-supported-agent` retrospective surfaced the gap. No follow-up needed — because the fix lives in the shared ceremony, every skill that links `shell-ceremony.md` (including `bill-feature-verify`, `bill-kotlin-code-review`, `bill-kmp-code-review`, all 27) inherits the enforcement automatically without per-skill mirror work.
Feature flag: N/A
...
- Relocated three quality-check SKILL.md files via `git mv` from `skills/<platform>/bill-<platform>-quality-check/` to `platform-packs/<platform>/quality-check/bill-<platform>-quality-check/` (agent-config, go, kotlin), added Description/EMR/TCH sections, and wired sibling `stack-routing.md` + `telemetry-contract.md` symlinks per relocated skill. `kmp` and `backend-kotlin` packs intentionally omit `declared_quality_check_file`; the shell hard-codes an explicit `kmp`/`backend-kotlin` → `kotlin` fallback route so existing KMP/backend-kotlin quality-check behavior stays identical.
