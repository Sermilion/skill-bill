...
- The envelope table lists top-level `uuid` as the delivery identity and keeps `$insert_id`
  as a legacy mirror.
- An unconfirmed delivery consumes an attempt, and five attempts block the row.
...
9. `docs/telemetry-privacy.md` names top-level `uuid` as the delivery identity, states the
   unconfirmed-attempt budget and the relay test-identity drop, and contains no claim that
