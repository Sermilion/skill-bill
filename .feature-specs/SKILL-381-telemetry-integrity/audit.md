...
- Current code still allows unbounded resends: `attemptDelivery` maps a thrown send or a
...
## F8. Swallowed sync failure

- `TelemetrySyncRuntime.autoSyncTelemetry` maps any `Exception` to `null` and emits
  nothing, against `docs/observability-policy.md`.
