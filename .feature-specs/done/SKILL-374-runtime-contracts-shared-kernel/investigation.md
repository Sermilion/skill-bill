...
| SKILL-376 runtime-infra | Subtask 3 changes infra scan prefixes in the same `packagePrefixForModule`, collapses infra:contracts packages under a 12-file ceiling, and requires every infra test package to exist in production | 376 for infra layout | 374.2 after 376.3 in the global order, although either order would work. The new `skillbill.infrastructure.contracts.locator` package is outside 376.3's collapse list, and the moved repoTests use existing production packages (376.3's rule exempts `src/repoTest`). 376's row says "no shared files", which is no longer true |
