...
1. `FeatureTaskRuntimeRunLoopContext` extension functions total the retained set named in `ARCHITECTURE.md` (forward drive loop, review-generation invalidation, launch capture, gate-cycle and fix-loop orchestration, review driver execution) and every other helper in the 22 files takes explicit request, state, recorder, goal-recorder, diagnostics, clock, or session parameters. An architecture test asserts the per-file census recorded in the document and fails when the total grows.
