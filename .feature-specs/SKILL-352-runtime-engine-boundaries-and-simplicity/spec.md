...
1. The count of `FeatureTaskRuntimeRunLoopContext` extension functions across every `FeatureTaskRuntimeRunLoop*.kt` file falls from 85 to the set the architecture document names as retained (forward drive loop, review-generation invalidation, launch capture, gate-cycle and fix-loop orchestration, review driver execution), and every other helper takes request, state, recorder, goal-recorder, diagnostics, clock, or session as explicit parameters. `ARCHITECTURE.md` records the per-file census for all 22 files and an architecture test asserts the total does not grow.
...
11. The 30 application types with no production consumer outside the engine live in the engine area that produces them; the CLI and MCP still reach IDE status through the pinned inbound API; `ARCHITECTURE.md` lists the remaining shared application types the engine imports.
...
14. Documentation describes the actual helper-input census, bag policy, persistence owner, key ownership, shared application edge, and scanner coverage. Existing architecture guards keep their protection with no new baseline rows, exemptions, or suppressions.
