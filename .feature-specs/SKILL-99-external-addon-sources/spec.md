...
- `platform-packs/ios/addons/capmo-*.md` — 12 files, **local-only, git-ignored** via a `.git/info/exclude` entry `platform-packs/ios/addons/capmo-*.md`.
- `platform-packs/ios/platform.yaml` — **skip-worktree** (`git ls-files -v` → `S`); holds the local `capmo` wiring. `addon_usage` block at L66.
- Note: `platform-packs/ios/addons/offline-*.md` (4 files) are **tracked/public** — they are NOT private and STAY in the repo. Only the `capmo-*` family + its wiring migrates out.
...
- addon markdown files (any names; e.g. `capmo-persistence.md`)
...
pointers:
  code-review/bill-ios-code-review-persistence:
    - name: capmo-persistence
      target: capmo-persistence.md          # source-relative; rewritten on merge
addon_usage:
  code-review/bill-ios-code-review-persistence:
    - slug: capmo-persistence
      entrypoint: capmo-persistence
      companion_pointers: []
```
...
Move the **`capmo-*`** private iOS review addon out of the skill-bill tree into an external source directory at a durable path outside `~/.skill-bill`:

- its per-area `capmo-*.md` files, plus an `addon-manifest.yaml` carrying their `addon_usage`/`pointers` wiring for the `ios` code-review skills;
- register the directory in `~/.skill-bill/config.json` `external_addon_sources`;
- **remove the in-tree workaround**: the `.git/info/exclude` entry `platform-packs/ios/addons/capmo-*.md`, the skip-worktree state on `platform-packs/ios/platform.yaml` (`git update-index --no-skip-worktree`), and the local `capmo` wiring inside that `platform.yaml` — so the working tree matches upstream and is clean;
...
7. The SKILL-98 private iOS addon (`capmo-*`) is migrated to an external source directory + `addon-manifest.yaml`, registered in `~/.skill-bill/config.json`; the in-tree workaround (`.git/info/exclude` `capmo-*` entry, skip-worktree on `platform-packs/ios/platform.yaml`, local `capmo` wiring in that file) is removed; the tracked public `offline-*.md` addons are untouched; and `git status` is clean with the working tree matching upstream.
